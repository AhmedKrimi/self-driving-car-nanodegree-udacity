...
def show_pcl(pcl):
    ...
    # define update_geometry function for the register_key_callback
    ...
    vis.register_key_callback(262, update_geometry)
    vis.run()
    #######
    ####### ID_S1_EX2 END #######     
...
def bev_from_pcl(lidar_pcl, configs):
    ...
    intensity_map[np.int_(lidar_pcl_top[:, 0]), np.int_(lidar_pcl_top[:, 1])] = lidar_pcl_top[:, 3] /(np.amax(lidar_pcl_top[:, 3]) - np.min(lidar_pcl_top[:, 3]))
